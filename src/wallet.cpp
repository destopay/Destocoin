...
/**
 * Fees smaller than this (in udeo) are considered zero fee (for transaction creation)
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 DEO.");
                    }

                    if (useIX) {
                        strFailReason += " " + _("SwiftTX requires inputs with at least 2 confirmations, you might need to wait a few minutes and try again.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-deo-address
                    CScript scriptChange;
