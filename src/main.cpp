...
#if defined(NDEBUG)
#error "Destocoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in udeo) are considered zero fee (for relaying and mining)
...
int64_t GetSubsidy(int nHeight)
{
  ...
  */
    ...
    // total money supply 322,105,000
    if (nHeight < 100)                              // 300,000,000
        nSubsidy = 3 * 1000000 * COIN;
    else if (nHeight < Params().LAST_POW_BLOCK())   // 10,000,200
        nSubsidy = 100002 * COIN;
    else if (nHeight < 5000)                        // 4,800
        nSubsidy = 1 * COIN;
    else if (nHeight < 25000)                       // 600,000
        nSubsidy = 30 * COIN;
    else if (nHeight < 100000)                      // 1,500,000
        nSubsidy = 20 * COIN;
    else if (nHeight < 1050000)                     // 9,500,000
        nSubsidy = 10 * COIN;
    else                                            // 5,250,000 x 2
    {
        // halving after block 1,050,000
        int halvings = (nHeight - 1050000) / Params().SubsidyHalvingInterval();
        ...
        nSubsidy = 5 * COIN;
        // Subsidy is cut in half every 1,050,000 blocks which will occur approximately every 2 years (1 minute blocks)
        nSubsidy >>= halvings;
    }
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // after PoW remove 10% for budget - allocate in masternode-budget
    if (nHeight > Params().LAST_POW_BLOCK())
    {
        return nSubsidy * (100 - Params().SubsidyBudgetPercentage()) / 100;
    }

    return nSubsidy;
}

int64_t GetMasternodePayment(int nHeight, int nMasternodeCount)
{
    int64_t nSubsidy = GetSubsidy(nHeight);

    // No rewards till masternode activation.
    if (nHeight <= Params().LAST_POW_BLOCK() || nSubsidy == 0)
        return 0;

    return nSubsidy * Params().SubsidyMasternodePercentage() / 100; // 80% of block reward
}
...
{
    RenameThread("destocoin-scriptch");
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu time=%d date=%s progress=%f  cache=%u\n",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if(!CheckProofOfStake(block, hashProofOfStake)) {
            ...
            // destocoin: adjust issue at juncture between POS and POW, POS fails hash test
            if (pindexPrev->nHeight == Params().LAST_POW_BLOCK()) {
                LogPrintf("INFO: CheckWork(): at juncture between proof-of-work and proof-of-stake - skip check because transition...");
            }
            else if (Params().NetworkID() == CBaseChainParams::MAIN) {
                if (pindexPrev->nHeight < 500) {
                    LogPrintf("INFO: CheckWork(): less than 500 - skip check because of initial staking instability in mainnet...");
                }
            }
            else {
                return false;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DEO: We use certain sporks during IBD, so check to see if they are
