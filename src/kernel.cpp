...
{
    // Destocoin will hash in the transaction hash and the index number in order to make sure each hash is unique
...
bool stakeTargetHit(uint256 hashProofOfStake, int64_t nValueIn, uint256 bnTargetPerCoinDay)
{
		...
		if (fDebug) {
			LogPrintf("\n** stakeTargetHit: hashProofOfStake=%s, bnCoinDayWeight=%s, bnTargetPerCoinDay=%s, bnCoinDayWeight * bnTargetPerCoinDay=%s, success=%d\n",
				hashProofOfStake.ToString().c_str(),
				bnCoinDayWeight.ToString().c_str(),
				bnTargetPerCoinDay.ToString().c_str(),
				(bnCoinDayWeight * bnTargetPerCoinDay).ToString().c_str(),
				uint256(hashProofOfStake) < bnCoinDayWeight * bnTargetPerCoinDay);
		}
...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock blockFrom, const CTransaction txPrev, const COutPoint prevout, unsigned int& nTimeTx, unsigned int nHashDrift, bool fCheck, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    //create data stream once instead of repeating it in the loop
		if (fDebug) {
			LogPrintf("** about to data stream");
		}
    CDataStream ss(SER_GETHASH, 0);
    ss << nStakeModifier;
    ...
    if (fCheck) {
		...
		if (fDebug) {
			LogPrintf("** starting to interate the hashing");
		}
    bool fSuccess = false;
    ...
    for (i = 0; i < (nHashDrift); i++) //iterate the hashing
    {
		...
		if (fDebug) {
			LogPrintf("** finished looping through the nHashDrift thing... did we pass? pass was %d and nHashDrift %d", fSuccess, nHashDrift);
		}
