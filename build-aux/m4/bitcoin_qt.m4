...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; destocoin-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build destocoin-qt GUI (default=auto, qt5 tried first)])],
    [
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  dnl is no longer emitted despite the patch.
  ...
  if test x$bitcoin_qt_got_major_vers = x5; then
    ...
    if test x$bitcoin_cv_static_qt = xyes; then
      ...
      if test x$TARGET_OS = xwindows; then
        ...
        QT_LIBS="-lQt5ClipboardSupport -lQt5CglSupport -lQt5ThemeSupport -lQt5GraphicsSupport -lQt5FontDatabaseSupport -lQt5AccessibilitySupport $QT_LIBS"
        LIBS="-lcups $LIBS"
...
dnl           BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}PlatformSupport],[main],,BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXPlatformSupport not found)))
dnl         fi
       fi
     fi
  else
